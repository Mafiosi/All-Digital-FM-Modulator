...
%---------------------------------------------
% Gains:
Ks = bin2dec('1000')/2^3;     % left + right: 0100b = 0.100b = 0.5 decimal 
Kd = bin2dec('1000')/2^3;     % left - right: 1000b = 1.000b = 1.0 decimal 
Kp = bin2dec('0000')/2^3;     % 19 kHz pilot: 0001b = 0.001b = 0.125 decimal 
...
lprk = max( lprk, -2^(Nbitsin-1) );
lmrk = min( lmrk, 2^(Nbitsin-1)-1 );
lmrk = max( lmrk, -2^(Nbitsin-1) );

% Check if there was saturation:
if ( max( lprk ) == 2^(Nbitsin-1)-1 || min( lprk ) == -2^(Nbitsin-1) )
  fprintf('*** Warning: saturation detected in the L+R channel\n');
end
% Check if there was saturation:
if ( max( lmrk ) == 2^(Nbitsin-1)-1 || min( lmrk ) == -2^(Nbitsin-1) )
  fprintf('*** Warning: saturation detected in the L-R channel\n');
end
